...
public enum VectorPolicyRequalification {
    ...
    private static func validateCheckpointLineage(
        ...
        guard path(
                parentDirectory,
                matchesManifestReference: manifest.parentCheckpointDirectory),
              !requirePreparedCandidatePath
                || path(
                    candidateDirectory,
                    matchesManifestReference:
    ...
    /// Match an actual checkpoint URL against a manifest path without making
    /// relative references depend on the process working directory. Checked-in
    /// evidence intentionally records repository-relative paths so bundles can
    /// be verified after the package is embedded or relocated.
    private static func path(
        _ actual: URL,
        matchesManifestReference reference: String
    ) -> Bool {
        if NSString(string: reference).isAbsolutePath {
            return standardizedPath(actual.path) == standardizedPath(reference)
        }
        let referenceComponents = reference.split(
            separator: "/", omittingEmptySubsequences: false).map(String.init)
        guard !referenceComponents.isEmpty,
              referenceComponents.allSatisfy({
                  !$0.isEmpty && $0 != "." && $0 != ".."
              }) else {
            return false
        }
        let actualComponents = actual.resolvingSymlinksInPath()
            .standardizedFileURL.pathComponents.filter { $0 != "/" }
        guard actualComponents.count >= referenceComponents.count else {
            return false
        }
        return Array(actualComponents.suffix(referenceComponents.count))
            == referenceComponents
    }
