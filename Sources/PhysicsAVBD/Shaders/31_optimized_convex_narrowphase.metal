    ...
    thread uint& edgePairTests)
{
    ...
    for (uint indexA = 0u; indexA < countA; indexA++) {
        ...
        if (deficitA <= supportTolerance) {
            if (supportingCountA == NPC_MAX_POLY_EDGES) return false;
    ...
    for (uint indexB = 0u; indexB < countB; indexB++) {
        ...
        if (deficitB <= supportTolerance) {
            if (supportingCountB == NPC_MAX_POLY_EDGES) return false;
    ...
    device const ConvexEdgeGPU* edges)
{
    ...
    // Two supported 120-vertex rounded hulls can each have 300 edges.
    // 128K covers their complete 90K-axis query without truncation.
