...
fragment ReconstructionOut reconstruction_fragment(VOut in [[stage_in]], constant Uniforms& U [[buffer(1)]],
                                                     constant ReconstructionUniforms& R [[buffer(2)]], constant MaterialResources& materials [[buffer(10)]]) {
...
fragment ReconstructionDisplayOut reconstruction_display_fragment(FSOut in [[stage_in]], constant Uniforms& U [[buffer(1)]],
    texture2d<float> color [[texture(0)]], depth2d<float> depth [[texture(4)]], texture3d<float> displayLUT [[texture(8)]]) {
    ReconstructionDisplayOut o;
    o.color = float4(displayColorSRGB8(displayTonemap(max(color.read(uint2(in.position.xy)).rgb,0.0), U, displayLUT),in.position.xy),1);
