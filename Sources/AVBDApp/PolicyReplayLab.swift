...
final class PolicyReplayModel: ObservableObject, RenderableModel {
    ...
    @Published private(set) var policyTrust:
        PolicyReplayRuntimeTrust = .unavailable
    ...
    var replayControlExplanation: String {
        switch policyTrust {
        case .unverifiedOperatorOverride, .developmentCheckpoint:
            return trustWarningPrefix + unverifiedReplayControlDetails
        default:
            return replayControlDetails
        }
    }
    private var unverifiedReplayControlDetails: String {
        switch robot.runtime {
        case .nativeMLX:
            return "Normal motion executes the selected MLX candidate through the task's ordinary observation and motor-action interfaces. No catalog evaluation or deployment identity is attributed to it."
        case .unitreeRecurrentMLX:
            return "The selected recurrent candidate executes in MLX. Its manifest-recorded vectors are checked only for internal consistency and do not establish provenance."
        case .externalReferenceMLX:
            return "The selected reference-conditioned candidate executes in native MLX; every rendered link still moves only through policy actions, motors, and physical contacts."
        case .classicalController:
            return replayControlDetails
        }
    }
    private var replayControlDetails: String {
        switch robot.runtime {
        case .nativeMLX:
        ...
        case .unitreeRecurrentMLX:
            return "This is a static public recurrent policy imported to MLX. It is not connected to the native trainer or checkpoint hot-reload path."
        case .externalReferenceMLX:
            return "This is a static GEAR-SONIC policy imported from ONNX to native MLX. The selected reference clip supplies commands; every rendered link still moves only through the policy, motors, and physical contacts."
        case .classicalController:
    ...
    private var trustWarningPrefix: String {
        switch policyTrust {
        case .unverifiedOperatorOverride:
            return "UNVERIFIED OPERATOR OVERRIDE: these bytes are not the catalog-commissioned packaged release. "
        case .developmentCheckpoint:
            return "UNVERIFIED DEVELOPMENT CHECKPOINT: these mutable or repository-local bytes do not inherit catalog qualification. "
        default:
            return ""
        }
    }
    ...
    var scenarioSummary: String {
        switch policyTrust {
        case .unverifiedOperatorOverride, .developmentCheckpoint:
            return trustWarningPrefix + unverifiedScenarioDescription
        case .unavailable:
            return "A replay task is selected, but no policy source is currently executing."
        default:
            return scenarioDescription
        }
    }

    private var unverifiedScenarioDescription: String {
        switch robot {
        case .unitreeH1:
            return "A caller-supplied recurrent candidate is executing on the Unitree H1 replay plant. Its manifest-recorded golden sequence is only a candidate self-check; no catalog source or parity identity is attributed to these bytes."
        case .gearSonicG1:
            return "A development reference-conditioned candidate is executing on the G1 replay plant. No catalog release or external-source identity is attributed to these bytes."
        case .humanoidIsaac, .humanoidIsaacGoal:
            return "A candidate native MLX policy is executing on the selected H1 task and physical plant. Catalog qualification evidence and sealed performance claims do not apply to these bytes."
        case .humanoidBoxCarry:
            return "A development H1 box-policy candidate is executing on the selected physical task. Only the live metrics shown below describe these bytes."
        case .arachne, .arachneGoal:
            return "A candidate native MLX policy is executing on the selected Arachne-15 task and physical plant. Catalog qualification evidence and sealed performance claims do not apply to these bytes."
        case .arachneClassical:
            return "The non-neural Arachne controller is executing on the selected physical plant."
        }
    }
    ...
    func rebuild() {
        ...
        do {
            ...
            let declaredEntry: PolicyReplayCatalogEntry?
            let checkpointRelativeDirectory: String
            if robot.usesClassicalController {
                declaredEntry = PolicyReplayCatalog.entry(
            ...
            // A packaged app never silently reaches back into its process CWD
            // when a sealed resource is missing or corrupt. Repository paths
            // remain a source-build convenience and are always development.
            let repositoryPath = Bundle.main.bundleURL.pathExtension == "app"
                ? nil : packagedPath
            let bundledPath = Bundle.main.resourceURL?
                .appendingPathComponent(
                    "checkpoints/\(checkpointRelativeDirectory)").path
            let configurationSources: [PolicyReplayCheckpointSource]
            if robot.usesClassicalController {
                configurationSources = []
            } else if let declaredEntry {
                configurationSources = PolicyReplayCheckpointResolution.sources(
                    explicit: overridePath,
                    live: autoLoadLatest ? liveRunDirectory : nil,
                    bundled: bundledPath, repository: repositoryPath,
                    entry: declaredEntry)
            } else {
                configurationSources = PolicyReplayCheckpointResolution
                    .candidates(
                        explicit: overridePath, fallbacks: [
                            autoLoadLatest ? liveRunDirectory : nil,
                            bundledPath, repositoryPath,
                        ]).map {
                            .init(
                                directory: $0,
                                origin: .repository,
                                trust: overridePath == nil
                                    ? .developmentCheckpoint
                                    : .unverifiedOperatorOverride)
                        }
            }
            let checkpointMetadata = configurationSources.lazy.compactMap {
                self.replayMetadata(
                    at: $0.directory, task: desiredTaskID)
            }.first
            ...
            if robot.usesClassicalController {
                ...
                policyTrust = .nonNeuralBaseline
                policyStatus = policyTrust.displayLabel
                    + " · paired-ripple CPG · exact IK · no checkpoint"
            }
            ...
            let candidates: [PolicyReplayCheckpointSource]
            if robot.usesClassicalController {
                candidates = []
            } else if let declaredEntry {
                candidates = PolicyReplayCheckpointResolution.sources(
                    explicit: overridePath,
                    live: startupLiveCheckpoint?.directory,
                    bundled: bundledPath, repository: repositoryPath,
                    entry: declaredEntry)
            } else {
                candidates = PolicyReplayCheckpointResolution.candidates(
                    explicit: overridePath, fallbacks: [
            ...
            for source in candidates
                where FileManager.default.fileExists(
                    atPath: "\(source.directory)/metadata.json") {
                do {
                    try installCheckpoint(
                        source.directory,
                        candidate: source.directory
                                == startupLiveCheckpoint?.directory
                            ? startupLiveCheckpoint : nil,
                        source: source)
                    break
                } catch {
                    loadFailures.append("\(source.directory): \(error)")
                }
            }
    ...
    private func installUnitreeH1Replay() throws {
        let overridePath = explicitCheckpointPath
        guard let entry = PolicyReplayCatalog.entry(
            selectionID: Robot.unitreeH1.selectionID),
              let releaseIdentity = entry.unitreeH1ReleaseIdentity else {
            throw RLEnvironmentError.invalidConfiguration(
                "Unitree H1 replay has no pinned catalog release identity")
        }
        let bundledPath = Bundle.main.resourceURL?
            .appendingPathComponent("checkpoints/external/unitree-h1").path
        let repositoryPath = Bundle.main.bundleURL.pathExtension == "app"
            ? nil : "checkpoints/external/unitree-h1"
        let candidates = PolicyReplayCheckpointResolution.sources(
            explicit: overridePath, live: nil, bundled: bundledPath,
            repository: repositoryPath, entry: entry)
        guard let source = candidates.first(where: {
            FileManager.default.fileExists(
                atPath: "\($0.directory)/manifest.json")
                && FileManager.default.fileExists(
                    atPath: "\($0.directory)/policy.safetensors")
        }) else {
            throw RLEnvironmentError.invalidConfiguration(
                "imported Unitree H1 policy not found; run "
                    + "Tools/import_unitree_h1_policy.py first")
        }
        let expectedIdentity = source.trust == .verifiedPackagedExternal
            ? releaseIdentity : nil
        let session = try UnitreeH1Sim2SimSession(
            policyDirectory: source.directory,
            expectedReleaseIdentity: expectedIdentity)
        unitreeH1 = session
        loadedCheckpointDirectory = source.directory
        policyTrust = source.trust
        unitreeInitialPosition = session.environment.state().root.position
        controlSteps = 0
        episodeFinished = false
        running = true
        policyStatus = policyTrust.displayLabel
            + " · executing converted weights "
            + String(session.policy.manifest.weightsSHA256.prefix(12))
            + "…\nmanifest-claimed source checkpoint "
            + String(session.policy.manifest.source.checkpointSHA256.prefix(12))
            + "… · \(source.directory)"
        trainingStatus = policyTrust.isTrustedPackagedRelease
            ? "externally pretrained policy · anchored release · recurrent MLX inference"
            : "externally pretrained candidate · provenance UNVERIFIED · recurrent MLX inference"
    }
    ...
    private func installGEARSonicG1Replay() throws {
        ...
        policyTrust = overridePath == nil
            ? .developmentCheckpoint : .unverifiedOperatorOverride
        policyStatus = policyTrust.displayLabel
            + " · loaded GEAR-SONIC G1 · candidate MLX/ONNX self-check "
    ...
    func pollForLatestCheckpoint(force: Bool = false) {
        ...
        let previousProvider = actionProvider
        let previousDirectory = loadedCheckpointDirectory
        let previousUpdate = loadedUpdate
        let previousTrust = policyTrust
        do {
            try installCheckpoint(
                candidate.directory, candidate: candidate,
                source: .init(
                    directory: candidate.directory, origin: .liveRun,
                    trust: .developmentCheckpoint))
            try restartEpisode()
        } catch {
            actionProvider = previousProvider
            loadedCheckpointDirectory = previousDirectory
            loadedUpdate = previousUpdate
            policyTrust = previousTrust
            policyStatus = policyTrust.displayLabel
                + " · new checkpoint rejected; replay kept previous policy: "
    ...
    private func installCheckpoint(
        ...
        source: PolicyReplayCheckpointSource
    ) throws {
        guard let task else { return }
        let loaded: any RLActionProvider
        let metadata: VectorPolicyMetadata
        if source.trust == .qualifiedPackagedNative {
            guard source.origin == .applicationBundle,
                  let entry = PolicyReplayCatalog.entry(
                    selectionID: robot.selectionID),
                  entry.qualification == .accepted,
                  let expectedTaskRevision = entry.expectedTaskRevision,
                  let expectedFingerprint =
                    entry.expectedCheckpointFingerprint,
                  let expectedManifestSHA256 =
                    entry.expectedDeploymentManifestSHA256 else {
                throw RLEnvironmentError.invalidConfiguration(
                    "qualified replay is missing its pinned deployment identity")
            }
            ...
            let runner = try VectorPolicyRunner(checkpointDirectory: path)
            loaded = runner
        ...
        policyTrust = source.trust
        let trustLabel = policyTrust.displayLabel
        if let candidate {
            loadedUpdate = candidate.completedUpdates
            policyStatus = trustLabel + " · loaded complete update "
                + "\(candidate.completedUpdates) · "
                + "\(candidate.environmentSteps.formatted()) environment steps\n\(path)"
        } else {
            loadedUpdate = trainingState(at: path)?.completedUpdates
            policyStatus = trustLabel + " · loaded \(path)/policy.safetensors"
        }
    }
    ...
    private func restartEpisode() throws {
        ...
        if robot == .unitreeH1 {
            ...
            let expectedIdentity: UnitreeH1ReleaseIdentity?
            if policyTrust == .verifiedPackagedExternal {
                guard let identity = PolicyReplayCatalog.entry(
                    selectionID: Robot.unitreeH1.selectionID)?
                    .unitreeH1ReleaseIdentity else {
                    throw RLEnvironmentError.invalidConfiguration(
                        "verified Unitree replay lost its catalog identity")
                }
        ...
        HSplitView {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    .pickerStyle(.menu)
                    Text(model.policyTrust.displayLabel)
                        .font(.caption.bold())
                        .foregroundStyle(
                            model.policyTrust.isTrustedPackagedRelease
                                || model.policyTrust == .nonNeuralBaseline
