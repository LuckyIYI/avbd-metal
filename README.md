...
| **[Divide and Truncate / Planar-DAT](https://arxiv.org/abs/2604.15513)** (Chen et al., 2026) | Direction-aware V–T/E–E truncation after prediction and every VBD color; OGC remains the contact-force model. Automatic mode selects Planar-DAT for shell-only scenes and opt-in volumetric self-contact, while ordinary closed tet bodies and mixed scenes use the lower-overhead isotropic bound. Its cross-solid bound ignores nonapproaching common-mode translation so nearby volumes can free-fall together. Signed tet-volume prevention is a separate experimental opt-in; rigid curved-trajectory and animated-DoF DAT are not implemented |
