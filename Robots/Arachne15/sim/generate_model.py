...
def generate(profile: str, mesh_directory: str) -> str:
    ...
    lines = [
        ...
        f'  <compiler angle="radian" meshdir="{mesh_directory}" autolimits="true"/>',
...
def main() -> None:
    ...
    if args.check and args.install_visual_meshes is not None:
        parser.error("--check cannot modify packaged visual meshes")
    if args.install_visual_meshes is not None:
        missing = [
            args.install_visual_meshes / filename
            for filename in VISUAL_MESH_FILES
            if not (args.install_visual_meshes / filename).is_file()
        ]
        if missing:
            raise SystemExit(
                "missing CAD visual meshes: "
                + ", ".join(str(path) for path in missing))
    mismatches = []
    outputs: list[tuple[Path, str]] = []
    for profile in ("training", "validation"):
        outputs.append((
            ROOT / f"arachne15_{profile}.xml",
            generate(profile, SOURCE_MESH_DIRECTORY),
        ))
        outputs.append((
            BUNDLED_ASSET_ROOT / f"arachne15_{profile}.xml",
            generate(profile, BUNDLED_MESH_DIRECTORY),
        ))
    ...
    for filename in VISUAL_MESH_FILES:
        ...
        if args.install_visual_meshes is not None:
            ...
            shutil.copyfile(args.install_visual_meshes / filename, destination)
            print(destination)
