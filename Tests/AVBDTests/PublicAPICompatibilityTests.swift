...
final class PublicAPICompatibilityTests: XCTestCase {
    ...
    func testLegacyManifoldFrictionSetsBothCoefficients() {
        let solver = CPUSolver()
        let bodyA = solver.addBody(
            size: F3(repeating: 1), density: 1, friction: 0.2,
            position: .zero)
        let bodyB = solver.addBody(
            size: F3(repeating: 1), density: 1, friction: 0.3,
            position: F3(0, 0, 2))
        let manifold = CPUManifold(solver: solver, bodyA: bodyA, bodyB: bodyB)

        manifold.friction = 0.55

        XCTAssertEqual(manifold.friction, 0.55)
        XCTAssertEqual(manifold.staticFriction, 0.55)
        XCTAssertEqual(manifold.dynamicFriction, 0.55)
    }
