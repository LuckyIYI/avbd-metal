...
extension Main {
 static func runSurface(field:ImplicitField,mode inputMode:String,count:Int,out:String) throws {
  ...
  for i in 0..<count {
   ...
   if mode=="surface-hull" {
    scene.colliders[c].collisionEnabled=false
    let asset=try JSONDecoder().decode(ConvexHullAsset.self,from:Data(contentsOf:URL(fileURLWithPath:CommandLine.arguments[1]).deletingLastPathComponent().appendingPathComponent("floor-hull.json")))
  ...
  let clock=Date();var trace=[[[Float]]]();var minimumZ=Float.infinity
  do { for step in 0..<240 {try solver.submitStep();try solver.synchronize();minimumZ=min(minimumZ,solver.bodyStates(moving).map{$0.position.z}.min()!);if step%12==0 {trace.append(solver.bodyStates(moving).map{[$0.position.x,$0.position.y,$0.position.z]})}}
  ...
  let passed=z.allSatisfy{$0.isFinite && (mode=="surface-hole" || mode=="surface-edge-miss" ? $0 < -1 : abs($0-0.02)<0.001)}
  let report:[String:Any]=["minimum_center_z":minimumZ,"mode":inputMode,"replicas":count,"passed":passed,"final_z":z,"step_seconds":seconds,"steps_per_second":240/seconds,"initialization_seconds":initialization,"trace":trace]
